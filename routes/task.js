...
router.post('/', async (req, res, next) => {
  ...
  try {
    const task = await createTask(title, due_date, description, userId, done);
    res.status(task.status);
    res.json({ message : task.data});
...
router.patch('/:id', async (req, res, next) => {
  ...
  try {
    const task = await updateTask(userId, id, title, due_date, description, done);
      res.status(task.status);
      res.json({ message : task.data});
